    ...
    for day in test_days:
        ...
        for gw, gdf in day_df.groupby(cfg.attraction_col):
            ...
            for infer_hour in range(PARK_OPEN_HOUR, PARK_CLOSE_HOUR):
                ...
                # optional debug: print a few aligned rows for manual verification
                if cfg.debug_sample and (infer_hour == 12) and (not debug_printed) and (not pred.empty) and (not truth_window.empty):
                    aligned = pd.concat(
